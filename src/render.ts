...
export function unitCard(u: Unit) {
  ...
  const dmgAs = u.weapon?.staff ? `${attackSpeed(u)} AS` : `${dmg} dmg · ${attackSpeed(u)} AS`
  return `<div class="card unitCard ${u.hp <= 0 ? 'dead' : ''}"><div class="portraitStack">${portrait}</div><div><div class="row space nameRow"><div class="name">${u.name}</div></div><div class="class">${u.displayCls} ${levelLabel(u)}${leader}${wt}${status}${buffText}</div><div class="small dmgAsLine">${dmgAs}</div>${itemSkillList}<div class="hpbar"><i style="width:${(100 * u.hp) / u.maxHp}%"></i></div><div class="small">HP ${u.hp}/${u.maxHp} · Hit ${u.weapon.staff ? '--' : hitRate(u, { weapon: { type: 'none' }, stats: { lck: 0, spd: 0, con: 99 } } as any)} · Avo ${avoid(u)} · Crit ${u.weapon.staff ? '--' : floor((u.weapon.crit || 0) + u.stats.skl / 2 + (u.heldItem?.crit || 0) + (u.skill?.crit || 0))}</div>${weaponStatHTML(u.weapon)}<div class="stats">${statHTML(u)}</div></div></div>`
}
